...
import { registerUser, verifyIdentity, resetPasswordByName } from "@/lib/auth-api";
...
export default function AuthPageShell({ defaultMode = "login" }: Props) {
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-[#FFFCFA] p-4">
            <div className="relative w-full max-w-3xl h-[500px] overflow-hidden rounded-[1.125rem] border border-[#f0e6dc] bg-white shadow-lg">
                                ...
                                <form className="space-y-3" onSubmit={handleSubmit}>
                                    ...
                                    <div className="flex items-center justify-between gap-4 text-[10px] text-[#a8856b]">
                                        ...
                                        <button
                                            ...
                                            className="cursor-pointer font-medium text-[#E8541C] hover:opacity-70 bg-transparent border-0 p-0 text-[10px]"
                                        >
                                            Lupa kata sandi?
                                        </button>
                                    </div>
...
function ForgotPasswordModal({ onClose }: { onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            {/* Modal */}
            <div className="relative z-10 w-80 max-w-[360px] bg-white rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-[#f0e6dc] overflow-hidden">
                <div className="h-1 bg-gradient-to-r from-[#FF6B35] to-[#E8541C]" />

                <div className="p-6">
                    {/* Header */}
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-[0.95rem] font-extrabold text-[#1a0e08]">
                        ...
                        <button
                            onClick={onClose}
                            className="w-6 h-6 rounded-full bg-[#f5ede3] flex items-center justify-center text-[#a8856b] hover:text-[#E8541C] hover:bg-[#FFF5EE] transition-colors border-0 cursor-pointer text-[12px]"
                        >
                            ✕
                        </button>
                    </div>
                    ...
                    {step === "identity" && (
                        <form onSubmit={handleVerify} className="flex flex-col gap-3">
                            <p className="text-[11px] text-[#a8856b] -mt-1 mb-1">
                                Masukkan email dan nama lengkap yang terdaftar.
                            </p>
                            <div className="space-y-1">
                                <label className="text-[10px] font-medium text-[#a8856b]">Email</label>
                                <input
                                    type="email"
                                    value={email}
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-medium text-[#a8856b]">Nama Lengkap</label>
                                <input
                                    type="text"
                                    value={name}
                    ...
                    {step === "reset" && (
                        <form onSubmit={handleReset} className="flex flex-col gap-3">
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-medium text-[#a8856b]">Password Baru</label>
                                <div className="relative">
                                    <input
                                        ...
                                        required
                                        className="w-full rounded-lg border border-[#f0e6dc] bg-[#fafaf8] px-3 py-1.5 pr-8 text-[11px] text-[#1a0e08] outline-none transition focus:border-[#E8541C] focus:ring-2 focus:ring-[rgba(232,84,28,0.1)]"
                                    />
                                    <button
                                        type="button"
                                        onClick={() => setShowPw(!showPw)}
                                        className="absolute right-2 top-1/2 -translate-y-1/2 text-[#a8856b] hover:text-[#E8541C] border-0 bg-transparent cursor-pointer text-[11px]"
                                    >
                                        {showPw ? "🙈" : "👁"}
                                    </button>
                                </div>
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-medium text-[#a8856b]">Konfirmasi Password</label>
                                <input
                                    type={showPw ? "text" : "password"}
                                    value={confirm}
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className="flex-1 rounded-lg py-2 text-[11px] font-semibold text-[#a8856b] border border-[#f0e6dc] bg-white cursor-pointer hover:bg-[#fafaf8] transition"
                                >
                                    ← Kembali
                                </button>
                                <button
                    ...
                    {step === "success" && (
                        <div className="text-center py-4">
                            ...
                            <p className="text-[0.85rem] font-bold text-[#1a0e08] mb-1">Password berhasil diubah!</p>
                            <p className="text-[11px] text-[#a8856b]">Modal akan menutup otomatis...</p>
                        </div>
                    )}
                    ...
                    {step !== "success" && (
                        <div className="flex items-center justify-center gap-1.5 mt-4">
                            {(["identity", "reset"] as const).map((s) => (
                                <div key={s} className={`h-1 rounded-full transition-all duration-300 ${step === s ? "w-5 bg-[#E8541C]" : "w-2.5 bg-[#f0e6dc]"}`} />
                            ))}
                        </div>
                    )}
                </div>
        ...
        <div className="mb-4 flex gap-1 rounded-lg border border-[#f0e6dc] bg-[#fafaf8] p-1">
            {(["login", "register"] as const).map((m) => (
                <button key={m} onClick={() => switchTo(m)}
                    className={`flex-1 rounded-md py-1.5 text-center text-[11px] font-medium transition ${mode === m ? "bg-white text-[#E8541C] shadow-sm" : "text-[#a8856b] hover:text-[#1a0e08]"}`}>
