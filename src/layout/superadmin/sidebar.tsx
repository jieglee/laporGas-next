...
import {
    LayoutDashboard, FileText, Users, ShieldCheck, LogOut,
} from "lucide-react";
...
export const useSuperadminSidebar = () => useContext(Ctx);

export function SuperadminSidebarProvider({ children }: { children: React.ReactNode }) {
...
export default function SuperadminSidebar() {
    ...
    const { expanded, setExpanded } = useSuperadminSidebar();

    const nama = session?.user?.name ?? "Superadmin";
    const inisial = nama.split(" ").slice(0, 2).map((w) => w[0]).join("").toUpperCase();

    const isActive = (path: string) =>
        path === "/superadmin" ? pathname === path : pathname?.startsWith(path);
    ...
    return (
        <aside
            ...
            className="fixed top-0 left-0 h-screen flex flex-col border-r z-40 overflow-hidden"
            style={{
        ...
        >
            ...
            <div className="flex items-center px-5 py-7 mb-2">
                <Link href="/superadmin" className="flex items-center gap-2" style={{ textDecoration: "none" }}>
                    <div className="flex items-center justify-center rounded-lg shrink-0" style={{ width: 32, height: 32 }}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" style={{ color: "#C0392B" }}>
                    ...
                    <span style={textStyle({
                        ...
                        Lapor<span style={{ color: "#C0392B" }}>Gas</span>
                        <span style={{
                            fontSize: "0.5rem",
                            fontWeight: 700,
            ...
            {/* Divider */}
            <div style={{ height: 1, background: "#1f1f1f", margin: "0 16px 12px" }} />
                    ...
                    return (
                        <Link
                            ...
                            style={{
                                ...
                                border: active ? "1px solid rgba(192,57,43,0.2)" : "1px solid transparent",
                            }}
                            onMouseEnter={(e) => {
            ...
            {/* User card */}
            <div className="p-3" style={{ borderTop: "1px solid #1f1f1f", paddingBottom: 70 }}>
                <div
